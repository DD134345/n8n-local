...
async function checkHealth() {
    ...
    try {
        ...
        setIndicator('card-n8n', res.ok ? 'green' : 'red', res.ok ? 'Hoạt động bình thường' : `Lỗi HTTP ${res.status}`);
    } catch {
        setIndicator('card-n8n', 'red', 'Không kết nối được');
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            const models = (data.models || []).map(m => m.name).join(', ') || 'chưa có model';
            setIndicator('card-ollama', 'green', `Hoạt động — ${models}`);
        } else {
            setIndicator('card-ollama', 'red', `Lỗi HTTP ${res.status}`);
        }
    } catch {
        setIndicator('card-ollama', 'red', 'Không kết nối được (kiểm tra Docker)');
    }
    ...
    if (state.config.n8nApiKey) {
        try {
            ...
            setIndicator('card-sheets', sheetsWf.length ? 'green' : 'yellow',
                sheetsWf.length ? `${sheetsWf.length} workflow dùng Sheets đang chạy` : 'Chưa có workflow Sheets nào active');
            const wpWf = active.filter(w => /publish/i.test(w.name));
            setIndicator('card-wordpress', wpWf.length ? 'green' : 'yellow',
                wpWf.length ? 'Workflow publish đang chạy' : 'Workflow publish chưa active');
        } catch {
            setIndicator('card-sheets', 'gray', 'Cần API key hợp lệ');
            setIndicator('card-wordpress', 'gray', 'Cần API key hợp lệ');
        }
    } else {
        setIndicator('card-sheets', 'gray', 'Nhập API key để kiểm tra');
        setIndicator('card-wordpress', 'gray', 'Nhập API key để kiểm tra');
    }
}
...
async function loadWorkflows() {
    ...
    if (!state.config.n8nApiKey) {
        tbody.innerHTML = '<tr><td colspan="5">Nhập n8n API key ở phần cấu hình để xem workflows</td></tr>';
        return;
    }
    try {
        ...
        filterSel.innerHTML = '<option value="">Tất cả workflows</option>' +
        ...
        tbody.innerHTML = state.workflows.map(w => `
            <tr>
                ...
                <td><a href="${state.config.n8nUrl}/workflow/${w.id}" target="_blank" class="btn btn-secondary btn-sm">Mở trong n8n</a></td>
            </tr>
        `).join('') || '<tr><td colspan="5">Không có workflow nào</td></tr>';
    } catch (e) {
        tbody.innerHTML = `<tr><td colspan="5">Lỗi: ${escapeHtml(e.message)} — kiểm tra URL và API key</td></tr>`;
    }
}
...
async function loadExecutions() {
    ...
    if (!state.config.n8nApiKey) {
        tbody.innerHTML = '<tr><td colspan="7">Nhập n8n API key để xem lịch sử chạy</td></tr>';
        return;
    }
    ...
    try {
        ...
        Object.entries(errCounts).forEach(([wfId, count]) => {
            ...
            if (cell) cell.innerHTML = `<span class="badge badge-error">${count} lỗi gần đây</span>`;
        });
        ...
        tbody.innerHTML = state.executions.map(e => {
            ...
            return `
                <tr>
                    ...
                    <td>${started ? started.toLocaleString('vi-VN') : '--'}</td>
                    <td>${stopped ? stopped.toLocaleString('vi-VN') : '--'}</td>
                    <td>${duration}</td>
            ...
            `;
        }).join('') || '<tr><td colspan="7">Không có bản ghi nào</td></tr>';
    } catch (e) {
        tbody.innerHTML = `<tr><td colspan="7">Lỗi: ${escapeHtml(e.message)}</td></tr>`;
    }
}
...
function renderFeatures() {
    ...
    tbody.innerHTML = state.features.map((f, i) => `
        <tr>
            ...
            <td><span class="badge badge-${f.priority}">${f.priority === 'high' ? 'Cao' : f.priority === 'medium' ? 'Trung bình' : 'Thấp'}</span></td>
            <td>
                <select onchange="updateFeatureStatus(${i}, this.value)">
                    <option value="pending" ${f.status === 'pending' ? 'selected' : ''}>Chờ xử lý</option>
                    <option value="in_progress" ${f.status === 'in_progress' ? 'selected' : ''}>Đang làm</option>
                    <option value="done" ${f.status === 'done' ? 'selected' : ''}>Hoàn thành</option>
                </select>
            </td>
            <td>${new Date(f.created).toLocaleDateString('vi-VN')}</td>
            <td><button class="btn btn-secondary btn-sm" onclick="deleteFeature(${i})">🗑️</button></td>
        </tr>
    `).join('') || '<tr><td colspan="7">Chưa có yêu cầu nào</td></tr>';
}
...
document.getElementById('add-feature').addEventListener('click', () => {
    ...
    if (!title) return alert('Nhập tên tính năng');
...
function renderLogs() {
    ...
    `).join('') || '<tr><td colspan="4">Chưa có log nào</td></tr>';
}
